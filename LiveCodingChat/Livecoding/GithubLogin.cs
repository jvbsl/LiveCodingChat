...
{
	public class GithubLogin:ILoginMethod
	{
		public GithubLogin ()
		{
		}
		...
		{
			Console.WriteLine ("Login GitHub");
			HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create ("https://www.livecoding.tv/accounts/github/login");
		...
		private void EndGetResponse(IAsyncResult res)
		{
			...
			using (System.IO.StreamReader sr = new System.IO.StreamReader (response.GetResponseStream ())) {
				data = sr.ReadToEnd ();
				data = HtmlHelper.getElement (data, "<form accept-charset=\"UTF-8\" action=\"/session\"");
			}
